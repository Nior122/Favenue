...
async function extractDirectUrls() {
  ...
  // Based on the webpage content I fetched earlier, I can extract the direct URLs
  // The pattern shows thumbnail URLs like: https://img.coomer.st/thumbnail/data/...
  // The direct URLs are: https://img.coomer.st/data/... (remove 'thumbnail/')
  ...
  try {
    console.log('🚀 Extracting direct URLs for profile upload...');
    console.log(`📸 Processing ${directUrls.length} direct image URLs`);

    // Step 1: Save URLs to file
    const urlsContent = directUrls.join('\n');
    const filePath = `../data/${profileId}/urls.txt`;
    writeFileSync(filePath, urlsContent);
    console.log(`💾 Saved ${directUrls.length} URLs to ${filePath}`);

    // Step 2: Create posts from URLs
    await createPostsFromUrls(profileId, directUrls);
    
    console.log(`🎉 Successfully uploaded ${directUrls.length} media items to profile: ${profileId}`);
