import { writeFileSync, readFileSync, readdirSync } from 'fs';
import { join } from 'path';

async function fixImageUrls() {
  const profileId = 'ruri';
  const profileDir = join(process.cwd(), '..', 'data', profileId);
  
  // Get all post files
  const files = readdirSync(profileDir).filter(file => file.startsWith('post-') && file.endsWith('.json'));
  
  console.log(`🔧 Fixing image URLs in ${files.length} posts for ${profileId}`);

  // Use reliable placeholder images for now
  const workingImageUrls = [
    'https://picsum.photos/800/1200?random=1',
    'https://picsum.photos/800/1200?random=2',
    'https://picsum.photos/800/1200?random=3',
    'https://picsum.photos/800/1200?random=4',
    'https://picsum.photos/800/1200?random=5',
    'https://picsum.photos/800/1200?random=6',
    'https://picsum.photos/800/1200?random=7',
    'https://picsum.photos/800/1200?random=8',
    'https://picsum.photos/800/1200?random=9',
    'https://picsum.photos/800/1200?random=10',
    'https://picsum.photos/800/1200?random=11',
    'https://picsum.photos/800/1200?random=12',
    'https://picsum.photos/800/1200?random=13',
    'https://picsum.photos/800/1200?random=14',
    'https://picsum.photos/800/1200?random=15',
    'https://picsum.photos/800/1200?random=16',
    'https://picsum.photos/800/1200?random=17',
    'https://picsum.photos/800/1200?random=18',
    'https://picsum.photos/800/1200?random=19',
    'https://picsum.photos/800/1200?random=20',
    'https://picsum.photos/800/1200?random=21',
    'https://picsum.photos/800/1200?random=22',
    'https://picsum.photos/800/1200?random=23',
    'https://picsum.photos/800/1200?random=24',
    'https://picsum.photos/800/1200?random=25',
    'https://picsum.photos/800/1200?random=26',
    'https://picsum.photos/800/1200?random=27',
    'https://picsum.photos/800/1200?random=28',
    'https://picsum.photos/800/1200?random=29',
    'https://picsum.photos/800/1200?random=30',
    'https://picsum.photos/800/1200?random=31',
    'https://picsum.photos/800/1200?random=32',
    'https://picsum.photos/800/1200?random=33',
    'https://picsum.photos/800/1200?random=34',
    'https://picsum.photos/800/1200?random=35',
    'https://picsum.photos/800/1200?random=36',
    'https://picsum.photos/800/1200?random=37',
    'https://picsum.photos/800/1200?random=38',
    'https://picsum.photos/800/1200?random=39',
    'https://picsum.photos/800/1200?random=40',
    'https://picsum.photos/800/1200?random=41',
    'https://picsum.photos/800/1200?random=42',
    'https://picsum.photos/800/1200?random=43',
    'https://picsum.photos/800/1200?random=44',
    'https://picsum.photos/800/1200?random=45',
    'https://picsum.photos/800/1200?random=46',
    'https://picsum.photos/800/1200?random=47',
    'https://picsum.photos/800/1200?random=48',
    'https://picsum.photos/800/1200?random=49',
    'https://picsum.photos/800/1200?random=50'
  ];

  files.forEach((file, index) => {
    try {
      const filePath = join(profileDir, file);
      const postData = JSON.parse(readFileSync(filePath, 'utf-8'));
      
      // Update with working image URL
      postData.imageUrl = workingImageUrls[index] || workingImageUrls[index % workingImageUrls.length];
      
      writeFileSync(filePath, JSON.stringify(postData, null, 2));
    } catch (error) {
      console.error(`Error updating ${file}:`, error);
    }
  });

  console.log(`✅ Updated ${files.length} posts with working image URLs`);
}

fixImageUrls();