...
**Latest Update (August 17, 2025)**: **CAPTION DISPLAY & AUTO-SHUFFLE COMPLETED**: Fixed Vercel deployment caption issue - Vercel API endpoints now correctly include post titles and descriptions from JSON data. Added automatic profile shuffling every 30 seconds with synchronized countdown timer and manual shuffle button. Posts display actual captions ("Gothic Elegance", "Midnight Vibes", etc.) instead of generic labels. All API endpoints (local development and Vercel production) now properly map title/description fields from post JSON files. Auto-shuffle works continuously with visual countdown and manual override functionality.
